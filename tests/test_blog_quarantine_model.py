...
async def test_blog_repo_quarantine_table_exists():
    ...
    assert {"id", "repo_name", "src_dir", "quarantined_at", "created_at", "updated_at"} <= cols
    # repo_name 唯一
...
async def test_blog_repo_quarantine_crud(db):
    row = BlogRepoQuarantine(
        ...
        quarantined_at=__import__("datetime").datetime.now(__import__("datetime").timezone.utc),
    )
    db.add(row)
    await db.flush()

    found = (
        (await db.execute(select(BlogRepoQuarantine).where(BlogRepoQuarantine.repo_name == "pub_abc.git")))
        .scalars()
        .first()
    )
