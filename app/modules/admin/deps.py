...
from app.modules.auth.security import decode_access_token  # 仅复用 pyjwt 解码函数
...
def create_admin_access_token(user: User) -> str:
    """签发后台 access token（15min）。独立 payload + type=admin 标记，防误喂给前台端点。"""
...
async def get_current_admin(
    ...
    # token_version：被提升（注销/改密）后 access 失效
