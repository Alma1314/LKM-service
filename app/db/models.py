...
def expires_at(days: float = 0, minutes: float = 0) -> str:
    ...
    return (datetime.datetime.now(datetime.UTC) + datetime.timedelta(days=days, minutes=minutes)).isoformat()
...
class User(Base):
    ...
    uploaded_files: Mapped[list[UserStorageItem]] = relationship(back_populates="owner_id")
...
class ColumnApplication(Base):
    ...
    user: Mapped[User] = relationship(foreign_keys=[user_id], back_populates="column_applications")
    reviewer: Mapped[User | None] = relationship(foreign_keys=[reviewer_id])
    column: Mapped[Column | None] = relationship(back_populates="application", uselist=False)
...
class Column(Base):
    ...
    application_id: Mapped[int | None] = mapped_column(ForeignKey("column_applications.id"), unique=True, nullable=True)
    title: Mapped[str] = mapped_column(String(80), nullable=False)
    description: Mapped[str] = mapped_column(String(300), nullable=False)
    cover_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[ColumnStatus] = mapped_column(String(20), nullable=False, default=ColumnStatus.ACTIVE)
    created_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)
    updated_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)
    ...
    posts: Mapped[list[ColumnPost]] = relationship(back_populates="column", cascade="all, delete-orphan")
...
class ColumnPost(Base):
    ...
    status: Mapped[ColumnPostStatus] = mapped_column(String(20), nullable=False, default=ColumnPostStatus.PUBLISHED)
    created_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)
    updated_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)
...
class ForumComment(Base):
    ...
    parent_id: Mapped[int | None] = mapped_column(ForeignKey("forum_comments.id"), nullable=True)
    like_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    created_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)

    post: Mapped[ForumPost] = relationship(back_populates="comments")
    user: Mapped[User] = relationship(back_populates="forum_comments")
    parent: Mapped[ForumComment | None] = relationship(remote_side=[id], back_populates="replies")
    replies: Mapped[list[ForumComment]] = relationship(back_populates="parent", cascade="all, delete-orphan")


class UserStorageItem(Base):
    __tablename__ = "user_storages"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    owner_id: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=False)
    showed_path: Mapped[str] = mapped_column(String(255), nullable=False)
    actual_path: Mapped[str] = mapped_column(String(255), nullable=False)
    file_metadata: Mapped[FileInfo] = mapped_column(JSON, nullable=False)
...
class BlogSeries(Base):
    ...
    status: Mapped[BlogSeriesStatus] = mapped_column(String(20), nullable=False, default=BlogSeriesStatus.ACTIVE)
    created_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)
    updated_at: Mapped[str] = mapped_column(Text, nullable=False, default=now_iso)

    owner: Mapped[User] = relationship(back_populates="blog_series")
    comments: Mapped[list[BlogComment]] = relationship(back_populates="series", cascade="all, delete-orphan")
    stars: Mapped[list[BlogStar]] = relationship(back_populates="series", cascade="all, delete-orphan")
...
class BlogComment(Base):
    ...
    parent: Mapped[BlogComment | None] = relationship(remote_side=[id], back_populates="replies")
    replies: Mapped[list[BlogComment]] = relationship(back_populates="parent", cascade="all, delete-orphan")
